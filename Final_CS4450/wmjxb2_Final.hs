...
-----------------------------------------------------------------
eval (Declare bindings body) env = eval body newEnv         -- 'bindings' = [(String,Exp)]
  where
	vars   = map fst bindings
	values = generateVals (map snd bindings) env
	newEnv = zip vars values
	
	{-newEnv = generateBindings (bindings) env --	(x, eval exp env) : env
	generateBindings :: [(String,Exp)] -> Env -> Env
	generateBindings [] env = env
	generateBindings ((x,exp):bindings) env = (x, (eval exp env)):(generateBindings bindings env)
	-}	
-----------------------------------------------------------------
...
---------------------------------------------------------------
{-
Helper function generateVals: Converts Exps to Values within the
environment, using the eval function, to generate declarations. Couldn't get this to work 
after changing configurations and trying different solutions, not completely positive
why the value is not stored in the environment after evaluation, and couldn't
debug the actual execution of the function well enough to determine where the flaw was 
-}

generateVals :: [Exp] -> Env -> [Value]
generateVals [] env     = []
generateVals (x:xs) env = (eval x env):(generateVals xs env)
